            ...
            if VULNERABILITIES_KEY in dependency.keys():
                ...
                for vulnerability_id in vulnerability_ids:
                    for cve_report in dependency.get(VULNERABILITIES_KEY):
                        cve_name = cve_report.get(NAME_KEY)
                        if cve_name not in cves:
                            cves.add(cve_name)
                            alerts += 1
                            cve_severity = Severity[str.upper(cve_report.get(SEVERITY_KEY))]
                            ...
                            concerned_software = OwaspDependencyCheckSummary\
                                .get_concerned_software(cve_report)
                            vulnerabilities.append(Vulnerability(name=cve_report.get(NAME_KEY),
                                                                 software=concerned_software,
                                                                 severity=cve_severity,
                                                                 description=cve_report.get(
                                                                     DESCRIPTION_KEY)))
